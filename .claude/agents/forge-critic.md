...
**When labeling multiple regulatory or policy violations**: Each distinct regulation, platform policy, or stakeholder-harm axis that is independently blocking must be a separate weakness entry. Do not bundle GDPR, CCPA, and ToS into a single blocking item. If a feature violates multiple regulations or infringes on different stakeholder interests, list each as its own blocking weakness with its own mitigation path. This precision prevents the confidence score from being artificially inflated by consolidation.
